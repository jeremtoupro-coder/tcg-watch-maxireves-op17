...
if (!response.ok) throw new Error(`Impossible de lister les namespaces KV: HTTP ${response.status}`);
const payload = await response.json() as CloudflareEnvelope<KvNamespace[]>;
if (!payload.success) throw new Error(`Cloudflare refuse la liste KV: ${JSON.stringify(payload.errors ?? [])}`);
const matches = payload.result.filter((namespace) => namespace.title === namespaceTitle);
if (matches.length !== 1) throw new Error(`Namespace KV '${namespaceTitle}' introuvable ou ambigu (${matches.length} résultat(s)).`);
...
if (process.env.DEPLOY_ACTIVE_STORES?.trim()) vars.ACTIVE_STORES = process.env.DEPLOY_ACTIVE_STORES.trim();

const cronExpression = process.env.DEPLOY_CRON?.trim();
const generatedConfig: WranglerConfig = {
  ...baseConfig,
  vars,
  kv_namespaces: [{ binding: "TCG_STATE", id: matches[0].id }],
  triggers: { crons: cronExpression ? [cronExpression] : [] }
};

await writeFile("wrangler.generated.json", `${JSON.stringify(generatedConfig, null, 2)}\n`, "utf8");
await mkdir(".wrangler/deploy", { recursive: true });
await writeFile(".wrangler/deploy/config.json", `${JSON.stringify({ configPath: "../../wrangler.generated.json" }, null, 2)}\n`, "utf8");

console.log(`Configuration générée: KV=${namespaceTitle}, monitoring=${vars.MONITORING_ENABLED}, writeState=${vars.WRITE_STATE}, discord=${vars.DISCORD_MODE}, publicAudit=${vars.ALLOW_PUBLIC_AUDIT}, cron=${cronExpression ?? "désactivé"}`);
